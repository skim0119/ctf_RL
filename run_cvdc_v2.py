...
def train_decentral(
    ...
    for i,traj in enumerate(agent_trajs):
        ...
        traj_buffer[j]["initial_state"].extend(traj[17])
        traj_buffer[j]["final_state"].extend(traj[18])
            ...
            tf.data.Dataset.from_tensor_slices(
                {
                    ...
                    "initial_state": np.stack(traj_buffer[atype]["initial_state"]).astype(np.float32),
                    "final_state": np.stack(traj_buffer[atype]["final_state"]).astype(np.float32)
                }
            )
            .shuffle(64)
...
def run_network(observations, env,initial_state,greedy=False):
    ...
    initial_states = [[] for i in range(num_agent_types)]
    avail_actions_array = []
    for i,observation_i in enumerate(observations):
        ...
        initial_states[type_assign].append(initial_state[i])
    ...
    actor_out, critic_out = network.run_network_decentral(observations, avail_actions,initial_states)
    ...
    return a1, vg1, vc1, phi1, psi1, log_logits1, reward_pred1, avail_actions_array, final_state
...
def TestNetwork(env,episodes):
    ...
    for i in range(episodes):
        ...
        while not terminated:
            a1, _, _, _, _, _, _, _,initial_state = run_network(o1, env,initial_state,greedy=True)
