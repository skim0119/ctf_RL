...
def build_network(input_hold, output_size=128, return_layers=False):
    ...
    # Block 2 : Convolution
    net = layers.convolution(inputs=net, num_outputs=64, kernel_size=3, stride=1, padding='VALID')
    _layers['CNN1'] = net
    net = layers.max_pool2d(net, 2)
    net = layers.convolution(inputs=net, num_outputs=64, kernel_size=2, stride=1, padding='VALID')
    _layers['CNN2'] = net
    net = layers.max_pool2d(net, 2)

    # Block 3 : Attention (with residual connection)
    net, att_layers = non_local_nn_2d(net, 32, pool=False, use_dense=False, normalize=True, name='non_local', return_layers=True)
    _layers['attention'] = att_layers['attention']
    _layers['NLNN'] = net

    # Block 4 : Feature Vector
    # net = tf.reduce_max(net, axis=-2) # Feature-wise max pool (removed: loose positional info)
