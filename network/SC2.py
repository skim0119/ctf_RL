...
class V4PPO(tf.keras.Model):
    ...
    def __init__(self, input_shape, action_size=5,
        ...
        self.actor_dense3 = layers.Dense(128, activation='relu')
        self.actor_dense4 = layers.Dense(action_size)
        ...
        self.critic_dense3 = layers.Dense(128, activation='relu')
        self.critic_dense4 = layers.Dense(1, activation='linear', use_bias=False)
    ...
    def call(self, inputs):
        # Actor
        net = self.actor_dense1(inputs)
        net = self.actor_dense2(net)
        net = self.actor_dense3(net)
        net = self.actor_dense4(net)
        actor = self.softmax(net)
        log_logits = self.log_softmax(net)

        # Critic
        net = self.critic_dense1(inputs)
        net = self.critic_dense2(net)
        net = self.critic_dense3(net)
        net = self.critic_dense4(net)
