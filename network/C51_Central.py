...
class V2Dist(tf.keras.Model):
    ...
    def __init__(self, input_shape, action_size=5,
        ...
        self.static_network = keras.Sequential([
            layers.Input(shape=[20,20,3]),
            layers.SeparableConv2D(
                filters=16, kernel_size=4, strides=2,
                padding='valid', depth_multiplier=16, activation='relu'),
            layers.Conv2D(filters=32, kernel_size=3, strides=2, activation='relu'),
            layers.Conv2D(filters=32, kernel_size=2, strides=2, activation='relu'),
            layers.Flatten(),
            layers.Dense(units=32, activation='relu'),])
        self.dynamic_network = keras.Sequential([
            layers.Input(shape=[20,20,3]),
            layers.Conv2D(filters=8, kernel_size=2, strides=2, activation='relu'),
            Non_local_nn(4),
            layers.Flatten(),
    ...
    def call(self, inputs):
        static = tf.gather(inputs, [0,1,3], axis=-1)
        dynamic = tf.gather(inputs, [2,4,5], axis=-1)
...
def loss(model, target_model, state, reward, done, next_state,
    ...
    #Tz = tf.minimum(model.v_max, tf.maximum(model.v_min, reward[:,None] + gamma * model.z * (1-done)))
    Tz = tf.minimum(model.v_max, tf.maximum(model.v_min, reward[:,None] + gamma * model.z))
    bj = (Tz - model.v_min) / model.delta_z 
    m_l, m_u = tf.math.floor(bj+1e-6).numpy().astype(int), tf.math.ceil(bj-1e-6).numpy().astype(int)
    #A = (z_next**(1-done)) * (m_u - bj)
    #B = (z_next**(1-done)) * (bj - m_l)
    A = z_next * (m_u - bj)
    B = z_next * (bj - m_l)
    ...
    #mse = tf.reduce_mean(tf.square(td_error))
