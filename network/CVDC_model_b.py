...
class Decentral(tf.keras.Model):
    ...
    def __init__(self, input_shape, action_space, atoms=128,
            prebuilt_layers=None, trainable=True,num_agent_types=1):
        ...
        beta = np.ones([atoms,num_agent_types], dtype=np.float32)
    ...
    def call(self, inputs): # Full Operation of the method
        ...
        beta = tf.transpose(tf.math.abs(tf.gather(self.beta,agent_types,axis=1)))
        wv = tf.transpose(tf.tile(self.sf_v_weight.weights[0],(1,agent_types.shape[0])))
        wq = self.sf_q_weight.weights[0]
        wv_neg = wv * (1.0-beta)
        reward_predict = tf.reduce_sum(tf.stop_gradient(phi) * wv_neg,axis=-1) # inverse approx
        reward_predict = tf.reshape(reward_predict, [-1])
        inv_critic = tf.reduce_sum(psi * wv_neg,axis=-1)
        ...
        psi_q_pos = tf.linalg.matmul(tf.stop_gradient(psi)*beta, wq)
        psi_v_neg = tf.reduce_sum(tf.stop_gradient(psi)* wv_neg,axis=-1,keepdims=True)
        # print(wq_pos.shape)
        # print(wv_neg.shape)
        # print(psi_q_pos.shape)
        # print(psi_v_neg.shape)
        # exit()
        # Filtered decoder
        _phi = phi*beta
...
def loss_ppo(model, state, old_log_logit, action, old_value, td_target, advantage, td_target_c, rewards, next_state, avail_actions,
        eps, entropy_beta, q_beta, psi_beta, decoder_beta, critic_beta, learnability_beta, reward_beta,type_assign):
    num_sample = state.shape[0]

    # Run Model
    pi, v = model([state, action, avail_actions,type_assign])
    pi_next, v_next = model([next_state, action, avail_actions,type_assign]) # dummy action
    actor = pi['softmax']
    psi = v['psi']
