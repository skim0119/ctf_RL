...
class PPO_SF(tf.keras.Model):
    ...
    def __init__(self, action_size=5, trainable=True, lr=1e-4, eps=0.2, entropy_beta=0.01, name='PPO',N=5):
        ...
        # self.successor_layer2 = layers.Dense(1, activation='linear', name='value_prediction', use_bias=False)
    ...
    def get_actor_variables(self):
        return self.get_feature_variables+self.phi_dense1.variables+self.phi_dense2.variables+self.actor_dense1.variables

    @property
    def get_phi_variables(self):
        return self.get_feature_variables+self.phi_dense1.variables+self.phi_dense2.variables+self.successor_layer.variables

    @property
    def get_psi_variables(self):
        return self.get_feature_variables+self.phi_dense1.variables+self.phi_dense2.variables+self.psi_dense1.variables+self.psi_dense2.variables

    def build_loss(self, old_log_logit, action, advantage, td_target, reward, phi):
        def _log(val):
