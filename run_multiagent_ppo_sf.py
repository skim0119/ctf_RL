...
def make_env(map_size):
    return lambda: gym.make("cap-v0", map_size=map_size, config_path=env_setting_path,)
...
gpu_options = tf.GPUOptions(
    per_process_gpu_memory_fraction=GPU_CAPACITY, allow_growth=True
)
config = tf.ConfigProto(
    gpu_options=gpu_options, log_device_placement=LOG_DEVICE, allow_soft_placement=True
)
...
with tf.device(device_ground):
    network = Network(
        input_shape=input_size, action_size=action_space, scope="ground", sess=sess
    )
with tf.device(device_air):
    network_air = Network(
        input_shape=input_size, action_size=action_space, scope="uav", sess=sess
    )
...
network.save(
    saver, MODEL_PATH + "/ctf_policy.ckpt", global_episodes
)  # It save both network
...
def train(
    ...
    for idx, traj in enumerate(trajs):
        ...
        _, advantages = gae(traj[2], traj[3], 0, gamma, lambd, normalize=False)
        td_target, _ = gae(
            traj[6], traj[7], np.zeros_like(traj[6][0]), gamma, lambd, normalize=False
        )

        traj_buffer["state"].extend(traj[0])
        traj_buffer["action"].extend(traj[1])
        traj_buffer["td_target"].extend(
            [(np.array(td) * d).tolist() for td, d in zip(td_target, traj[8])]
        )
...
def get_action(states, N=16):
    ...
    (
        ...
        phi_ground,
        psi_ground,
    ) = network.run_network(blue_ground)
    action_air, value_air, logit_air, phi_air, psi_air = network_air.run_network(
        blue_air
    )

    action_rsh = np.concatenate(
        [action_air.reshape([NENV, 2]), action_ground.reshape([NENV, 4])], axis=1
    )
    value = np.concatenate(
        [value_air.reshape([NENV, 2]), value_ground.reshape([NENV, 4])], axis=1
    )
    logit = np.concatenate(
        [logit_air.reshape([NENV, 2, 5]), logit_ground.reshape([NENV, 4, 5])], axis=1
    )
    phi = np.concatenate(
        [phi_air.reshape([NENV, 2, N]), phi_ground.reshape([NENV, 4, N])], axis=1
    )
    psi = np.concatenate(
        [psi_air.reshape([NENV, 2, N]), psi_ground.reshape([NENV, 4, N])], axis=1
    )
    ...
    (
        ...
        phi_ground,
        psi_ground,
    ) = network.run_network(red_ground)
    action_air, value_air, logit_air, phi_air, psi_air = network_air.run_network(
        blue_air
    )

    action_rsh = np.concatenate(
        [action_rsh, action_air.reshape([NENV, 2]), action_ground.reshape([NENV, 4])],
        axis=1,
    )
    value = np.concatenate(
        [value, value_air.reshape([NENV, 2]), value_ground.reshape([NENV, 4])], axis=1
    )
    logit = np.concatenate(
        [logit, logit_air.reshape([NENV, 2, 5]), logit_ground.reshape([NENV, 4, 5])],
        axis=1,
    )
    phi = np.concatenate(
        [phi, phi_air.reshape([NENV, 2, N]), phi_ground.reshape([NENV, 4, N])], axis=1
    )
    psi = np.concatenate(
        [psi, psi_air.reshape([NENV, 2, N]), psi_ground.reshape([NENV, 4, N])], axis=1
    )
...
while global_episodes < total_episodes:
    ...
    is_air = np.array([agent.is_air for agent in envs.get_team_blue().flat]).reshape(
        [NENV, num_blue]
    )
    is_air_red = np.array([agent.is_air for agent in envs.get_team_red().flat]).reshape(
        [NENV, num_red]
    )
    is_air = np.concatenate([is_air, is_air_red], axis=1).reshape([-1])

    trajs = [
        Trajectory(depth=9) for _ in range((num_blue + num_red) * NENV)
    ]  # Trajectory per agent

    # Bootstrap
    s1 = envs.reset(
        config_path=env_setting_path
    )  # , custom_board='fair_uav/board_0001.txt')
    ...
    for step in range(max_ep + 1):
        ...
        is_alive = np.array(
            [agent.isAlive for agent in envs.get_team_blue().flat]
        ).reshape([NENV, num_blue])
        is_alive_red = np.array(
            [agent.isAlive for agent in envs.get_team_red().flat]
        ).reshape([NENV, num_red])
        ...
        pre_air_reward = (
            np.isin(s1[::6, :, :, -12], -1).sum(axis=2).sum(axis=1) * (-0.1)
            + np.isin(s1[::6, :, :, -10], [1, -1]).astype(int).sum(axis=2).sum(axis=1)
            * 10.0
        ) * np.repeat(np.array(~was_done, dtype=int), 2)
        air_reward = (
            np.isin(s1[::6, :, :, -6], -1).sum(axis=2).sum(axis=1) * (-0.1)
            + np.isin(s1[::6, :, :, -4], [1, -1]).astype(int).sum(axis=2).sum(axis=1)
            * 10.0
        ) * np.repeat(np.array(~was_done, dtype=int), 2)
        for idx in range(NENV * (num_blue + num_red)):
            ...
            if was_alive[idx] and not was_done[env_idx]:
                if is_air[idx]:
                    agent_reward = (
                        air_reward[env_team_idx] - pre_air_reward[env_team_idx]
                    )
                else:
    ...
    for env_id in range(NENV):
        steps.append(
            max(
                [
                    len(traj)
                    for traj in trajs[env_id * num_blue : (env_id + 1) * num_blue]
                ]
            )
        )
