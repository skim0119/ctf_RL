...
from network.model_V2 import V2_PPO_Termination
...
class PPO:
        ...
        **kwargs
        ...
        with self.sess.as_default(), self.sess.graph.as_default():
            with tf.variable_scope(scope):
                ...
                self.term_ = tf.placeholder(shape=[None], dtype=tf.int32, name='term_hold')
                self.term_logits_ = tf.placeholder(shape=[None, 2], dtype=tf.float32, name='term_old_logit_hold')

                # Build Network
                model = V2_PPO_Termination(action_size);  self.model = model
                self.actor, self.logits, self.log_logits, self.critic, self.termination = model(self.state_input)
                loss = model.build_loss(self.old_logits_, self.action_, self.advantage_, self.td_target_,self.term_,self.term_logits_)
    ...
    def run_network(self, states, return_action=True):
        ...
        a_probs, critics, logits, termination = self.sess.run([self.actor, self.critic, self.log_logits, self.termination], feed_dict)
        if return_action:
            actions = np.array([np.random.choice(self.action_size, p=prob / sum(prob)) for prob in a_probs])
            return actions, critics, logits, termination
        else:
            return a_probs, critics, logits, termination

    def update_network(self, state_input, action, td_target, advantage, old_logit,termination,term_logit, global_episodes, writer=None, log=False):
        feed_dict = {self.state_input: state_input,
                     self.action_: action,
                     self.td_target_: td_target,
                     self.advantage_: advantage,
                     self.old_logits_: old_logit,
                     self.term_: termination,
                     self.term_logits_: term_logit,}
