...
from network.CVDC_model_b import Central
from network.CVDC_model_b import Decentral as Decentral
from network.CVDC_model_b import train
from network.CVDC_model_b import loss_central
from network.CVDC_model_b import loss_ppo
...
class SF_CVDC:
    ...
    @tf.function(experimental_relax_shapes=True)
    def run_network_decentral(self, observations, avail_actions,agent_types):
        #(TODO) heterogeneous agent
        model = self.dec_models[0]
        num_sample = observations.shape[0]
        temp_action = np.ones([num_sample], dtype=int)
        actor, SF = model([observations, temp_action, avail_actions,agent_types])
        return actor, SF
        '''
        results = []
