...
TRAIN_NAME_ATT='ATT_9'
LOG_PATH_ATT='./logs/'+TRAIN_NAME_ATT
MODEL_PATH_ATT='./model/' + TRAIN_NAME_ATT

# Create model and log directory
if not os.path.exists(MODEL_PATH_ATT):
    raise OSError(f'Creation of the directory {MODEL_PATH_ATT} failed')
if not os.path.exists(LOG_PATH_ATT):
    raise OSError(f'Creation of the directory {LOG_PATH_ATT} failed')

TRAIN_NAME_NAV='NAV'
LOG_PATH_NAV='./logs/'+TRAIN_NAME_NAV
MODEL_PATH_NAV='./model/' + TRAIN_NAME_NAV

# Create model and log directory
if not os.path.exists(MODEL_PATH_NAV):
    raise OSError(f'Creation of the directory {MODEL_PATH_NAV} failed')
if not os.path.exists(LOG_PATH_NAV):
    raise OSError(f'Creation of the directory {LOG_PATH_NAV} failed')
...
class Worker(object):
    def __init__(self, name, global_step=0):
        ...
        self.AC_att = AC(in_size=in_size,
                     ...
                     entropy_beta = 0.01,
                     sess=sess_att,
                     global_network=global_ac_att)
        
        self.subgraph_nav = Subgraph(
            in_size=in_size,
            model_name=self.name,
            network_type=AC,
            sess=sess_nav,
            global_network=global_ac_nav
        )
        '''self.AC_nav = AC(in_size=in_size,
                     action_size=action_space,
                     lr_actor=lr_a,
                     lr_critic=lr_c,
                     scope=self.name,
    ...
    def get_action(self, states):
        actions_att, value_att = self.AC_att.run_network(states) 
        actions_nav, value_nav = self.subgraph_nav.get_action(states) 

        return actions_att[0:2]+actions_nav[2:], value_att[0:2].tolist()+value_nav[2:].tolist()

    def work(self, writer):
        ...
        while not coord.should_stop() and global_episodes < total_episodes:
            ...
            trajs_nav = [Trajectory(depth=4) for _ in range(2)]
            trajs_att = [Trajectory(depth=4) for _ in range(2)]
            ...
            for step in range(max_ep+1):
                ...
                if total_step % update_frequency == 0 or d:
                    self.train_att(trajs_att, v1[0:2])
                    self.subgraph_nav.train(trajs_att, v1[2:])
                    #self.train_nav(trajs_nav, v1[2:])
                    trajs_att = [Trajectory(depth=4) for _ in range(2)]
                    trajs_nav = [Trajectory(depth=4) for _ in range(2)]
...
#                saver_att.save(sess_att, MODEL_PATH_ATT+'/ctf_policy.ckpt', global_step=global_episodes)

    def train_att(self, trajs, bootstrap=0.0):
        ...
        feed_dict = {
            self.AC_att.state_input : np.stack(buffer_s),
            self.AC_att.action_ : np.array(buffer_a),
            self.AC_att.td_target_ : np.array(buffer_tdtarget),
            self.AC_att.advantage_ : np.array(buffer_adv),
        }

        # Update Buffer
        aloss, closs, etrpy = self.AC_att.update_global(
            buffer_s,
            buffer_a,
            buffer_tdtarget,
            buffer_adv
...
with sess_att.as_default(), sess_att.graph.as_default():
    global_ac_att = AC(in_size=in_size, action_size=action_space, scope=global_scope, sess=sess_att)
    global_ac_att.initialize_vars()
with sess_nav.as_default(), sess_nav.graph.as_default():
    global_ac_nav = AC(in_size=in_size, action_size=action_space, scope=global_scope, sess=sess_nav)
    global_ac_nav.initialize_vars()
...
for idx in range(nenv):
    name = 'W_%i' % idx
    workers.append(Worker(name, global_step=global_step))
    print(f'worker: {name} initiated')
writer = tf.summary.FileWriter(LOG_PATH, sess.graph)
    
with sess_att.as_default(), sess_att.graph.as_default():
    saver_att = tf.train.Saver(var_list=global_ac_att.get_vars)
    ckpt = tf.train.get_checkpoint_state(MODEL_PATH_ATT)
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        saver_att.restore(sess_att, ckpt.model_checkpoint_path)
...
with sess_nav.as_default(), sess_nav.graph.as_default():
    saver_nav = tf.train.Saver(var_list=global_ac_nav.get_vars)
    ckpt = tf.train.get_checkpoint_state(MODEL_PATH_NAV)
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        saver_nav.restore(sess_nav, ckpt.model_checkpoint_path)
        print("Load Model : ", ckpt.model_checkpoint_path)
        iuv(sess_nav)
    else:
        raise NotImplementedError
