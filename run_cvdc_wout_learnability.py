...
TRAIN_TAG = "Central value decentralized control(without learnability), " + TRAIN_NAME
...
elif os.path.exists(os.path.join('config','CVDC_SFOnly_'+args.map+'.yaml')):
    # Model path in config directory
    config_path = os.path.join('config', 'CVDC_SFOnly_'+args.map+'.yaml')
else:
    # Default config
