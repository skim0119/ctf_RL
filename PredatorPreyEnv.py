...
class StagHunt(gym.Env):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        shape = getattr(args, "world_shape", [10,10])
        ...
        self.agent_move_block = np.asarray(getattr(args, "agent_move_block", [0,1,2]), dtype=int_type)
        self.n_actions = 10 if self.directed_observations and self.directed_exta_actions \
            else (6 if self.capture_action else 5)
        self.n_agents = getattr(args, "n_agents", 8)
        self.n_stags = getattr(args, "n_stags", 8)
        self.p_stags_rest = getattr(args, "p_stags_rest", 0.0)
        self.n_hare = getattr(args, "n_hare", 0)
        self.p_hare_rest = getattr(args, "p_hare_rest", 0.0)
        self.n_prey = self.n_stags + self.n_hare
        self.agent_obs = getattr(args, "agent_obs", (2, 2))
        ...
        if self.observe_state:
            ...
            self.obs_size = self.n_feats * (2 * self.agent_obs[0] - 1) * (2 * self.agent_obs[1] - 1)
        else:
        ...
        self.time_reward = getattr(args, "reward_time", 0.0)
        self.collision_reward = getattr(args, "reward_collision", 0.0)
        self.capture_hare_reward = getattr(args, "reward_hare", 1.0)
        self.capture_stag_reward = getattr(args, "reward_stag", 10.0)
        self.miscapture_punishment = float(getattr(args, "miscapture_punishment", -2.0))
    ...
    def reset(self):
        ...
        self.agents_orientation = np.random.randint(low=0, high=3+1, size=(self.n_agents, self.batch_size))
    ...
    def step(self, actions):
        ...
        printTiming=False
        if printTiming: print("Initializing Time: {}".format( time.time()-ts));ts=time.time()
        ...
        if printTiming: print("Moving Agents: {}".format( time.time()-ts));ts=time.time()
        # Move the prey
        for b in range(self.batch_size):
            for p in np.random.permutation(self.n_prey):
                if self.prey_alive[p, b] > 0:
                    ...
                    grid = np.full((1,12,12,3),-1)
                    grid[:,1:-1,1:-1,:] = self.grid
                    if printTiming: print("   Creating Grid: {}".format( time.time()-ts));ts=time.time()
                    local_area = grid[b,self.prey[p, b, :][0]:self.prey[p, b, :][0]+3,self.prey[p, b, :][1]:self.prey[p, b, :][1]+3,:]
                    if printTiming: print("   LocalArea: {}".format( time.time()-ts));ts=time.time()
                    for u,pos in enumerate([[1,2], [2,1], [1,0], [0,1]]):
                        if local_area[pos[0],pos[1],0] == 1:
                            next_to_agent = True
                        if sum(np.abs(local_area[pos[0],pos[1],:]))<1.0:
                            possible.append(u)
                    if printTiming: print("   Analyzing Actions: {}".format( time.time()-ts));ts=time.time()
                    ...
                    if printTiming: print("   Capturing Mechanics: {}".format( time.time()-ts));ts=time.time()
                        ...
                        if not rest:
                            ...
                            if not (np.random.rand() < self.mountain_slope
                                ...
                                self.prey[p, b, :], _ = self._move_actor(self.prey[p, b, :], u, b,
                                                                         ...
                                                                         self.prey_type[p, b])
                    if printTiming: print("   Moving Prey ETC: {}".format( time.time()-ts));ts=time.time()
                    if printTiming: print(" -- ")
        ...
        if printTiming: print("Moving Prey: {}".format( time.time()-ts));ts=time.time()
        ...
        obs=self.get_obs()
        if printTiming: print("Generating Observation Time: {}".format( time.time()-ts))
        if self.batch_mode:
            return obs, reward, terminated, info
        else:
            done = [terminated[0] for i in range(self.n_agents)]
            return obs, reward[0].item(), np.asarray(done), info
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        if self.capture_action and self.agents_not_frozen[agent_id] > 0:
            ...
            grid = np.full((1,12,12,3),-1)
            grid[:,1:-1,1:-1,:] = self.grid
            local_area = grid[0,self.agents[agent_id, 0, :][0]:self.agents[agent_id, 0, :][0]+3,self.agents[agent_id, 0, :][1]:self.agents[agent_id, 0, :][1]+3,:]
            for pos in [[1,2], [2,1], [1,0], [0,1]]:
                if local_area[pos[0],pos[1],1] == 1:
    ...
    def get_env_info(self):
        info = {"state_shape": self.get_state_size(),
...
# ---------- PRIVATE METHODS ---------------------------------------------------------------------------------------
    ...
    def _observe(self, agent_ids):
        grid = np.full((1,14,14,3),-1)
        grid[:,2:-2,2:-2,:] = self.grid
        # grid = np.pad(self.grid,pad_width=((0,0),(self.agent_obs[0],self.agent_obs[0]),(self.agent_obs[1],self.agent_obs[1]),(0,0)),constant_values=-1.0)

        x = self.agents[agent_ids, 0, :]
        obs = grid[0,x[0][0]:x[0][0]+1+2*self.agent_obs[0],x[0][1]:x[0][1]+1+2*self.agent_obs[1],:].reshape(len(agent_ids), self.batch_size, -1)
        if self.batch_mode:
            return obs
        else:
            return obs[:, 0, :].squeeze()

    def _observe_(self, agent_ids):
...
if __name__ == '__main__':
    ...
    if True:
        ...
        while True:
            o, r, t, i = env.step((np.random.rand(env.n_agents) * 5) // 1)
            print(r)
            if r > 0:
                break
