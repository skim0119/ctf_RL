...
from method.CVDC_PPO import PPO_CVDC as Network

parser = argparse.ArgumentParser(description="CVDC(PPO without SF/Learnability) trainer for convoy")
...
elif os.path.exists(os.path.join('config','CVDC_PPO_'+args.map+'.yaml')):
    # Model path in config directory
    config_path = os.path.join('config', 'CVDC_PPO_'+args.map+'.yaml')
else:
    # Default config
...
while global_steps < total_steps:
    ...
    while episode < rollout_episodes:
        ...
        # Bootstrap
        a1, vg1, log_logits1 = run_network(o1, _avail_actions)
