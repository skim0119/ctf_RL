...
class RoombaV2(Policy):
    ...
    def initiate(self, free_map, agent_list):
        ...
        self.flag_range = 10 # Range to see the flag
    ...
    def gen_action(self, agent_list, observation, free_map=None):
        ...
        obsx, obsy = observation.shape
        padding = max(self.enemy_range, self.flag_range)
        _obs = np.empty((obsx+2*padding, obsy+2*padding)).astype(int)
        _obs[:] = 8 # Obstacle
        _obs[padding:obsx+padding, padding:obsy+padding] = observation
        obs = _obs
