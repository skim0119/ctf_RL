...
if OVERRIDE:
    ...
    if os.path.exists(LOG_PATH):
        print('Override existing log saves')
        shutil.rmtree(LOG_PATH,ignore_errors=True)
    if os.path.exists(MODEL_PATH):
        print('Override existing model saves')
...
## Training
total_episodes = 4e5#config.getint('TRAINING','TOTAL_EPISODES')
...
progbar = tf.keras.utils.Progbar(total_episodes,interval=100)
    ...
    def work(self, saver, writer):
        ...
        with self.sess.as_default(), self.sess.graph.as_default():
            while not coord.should_stop() and global_episodes < total_episodes:
                # select red
                if global_episodes < 1e5:
                    policy_red = policy_red_zeros
                elif global_episodes < 1.5e5:
