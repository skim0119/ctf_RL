...
from method.CVDC_b import SF_CVDC as Network
...
def train_decentral(
    ...
    for traj in agent_trajs:
        ...
        traj_buffer["type_assign"].extend(traj[17])
            ...
            tf.data.Dataset.from_tensor_slices(
                {
                    ...
                    "type_assign": np.stack(traj_buffer["type_assign"]).astype(np.int),
...
def run_network(observations, env,greedy=False):
    ...
    for i in range(num_agent):
        type_assign.append(agent_types_assign.index(agent_types[i]))
        avail_action = env.get_avail_agent_actions(i)
        avail_actions.append(avail_action)
    avail_actions = np.array(avail_actions)
    type_assign = np.array(type_assign)

    # Run decentral network
    observations = np.asarray(observations)
    actor, critic = network.run_network_decentral(observations, avail_actions,type_assign)
    ...
    return a1, vg1, vc1, phi1, psi1, log_logits1, reward_pred1, avail_actions,type_assign
...
def TestNetwork(env,episodes):
    ...
    for i in range(episodes):
        ...
        while not terminated:
            a1, _, _, _, _, _, _, _, _ = run_network(o1, env,greedy=True)
...
#while True:
    ...
    while dec_batch_size < buffer_size:
        ...
        dec_trajs = [Trajectory(depth=18) for _ in range(num_agent)]
        ...
        a1, vg1, vc1, phi1, psi1, log_logits1, reward_pred1, _avail_actions,type_assign = run_network(o1, env)
