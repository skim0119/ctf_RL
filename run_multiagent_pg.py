...
from method.pg import PG_Module as Network
...
TRAIN_NAME = "IPG_{}_{:02d}_convoy_{}g{}a_{}g{}a_m{}".format(
    args.machine,
    args.train_number,
    args.nbg,
    args.nba,
...
TRAIN_TAG = "PG e2e model w Stacked Frames: " + TRAIN_NAME
...
def train(
    ...
    for trajs in trajs:
        for idx, traj in enumerate(trajs):
            ...
            reward = traj[2]
            discounted_reward = discount(traj[2], gamma)
            advantages = discounted_reward

            advantage_list.append(discounted_reward)
...
def get_action(states):
    ...
    # Postprocessing
    for (a, p), mask in zip(results, agent_type_masking):
        a1[mask] = a
        p1[mask, :] = p
    actions = np.reshape(a1, [NENV, num_blue])
    return actions, a1, p1
        ...
        for env_idx in range(NENV):
            for agent_id in range(num_agent):
                ...
                trajs[env_idx][agent_id].append(
                    [s0[idx], a0[idx], reward[env_idx], 0, p0[idx], 0]
                )
